export interface Destination {
  id: string;
  name: string;
  types: ('beach' | 'waterfall' | 'cave' | 'rock')[];
  location: string;
  description: string;
  mainImage: string;
  images: string[];
  rating: number;
  reviews: number;
  price?: number;
  openingHours?: string;
  bestTimeToVisit?: string;
  facilities: string[];
  activities: string[];
  tips: string[];
  nearbyAttractions: {
    name: string;
    distance: string;
  }[];
}

export const destinationsData: Destination[] = [
  {
    id: "pantai-ciantir",
    name: "Pantai Ciantir",
    types: ["beach"],
    location: "Pantai Ciantir, Sawarna",
    description: "Pantai Ciantir adalah salah satu destinasi wisata unggulan di Desa Sawarna, Kecamatan Bayah, Kabupaten Lebak, Banten. Pantai ini menawarkan keindahan alami yang masih terjaga dengan baik. Hamparan pasir putih yang luas, laut biru yang jernih, dan ombak yang cukup besar menjadikan Pantai Ciantir sebagai tempat favorit bagi wisatawan yang ingin menikmati ketenangan alam atau aktivitas seru di tepi laut. Keunikan pantai ini terletak pada suasananya yang masih alami, jauh dari hiruk-pikuk kota, serta didukung oleh lingkungan masyarakat lokal yang ramah. Pantai Ciantir juga sering disebut sebagai surga tersembunyi di selatan Banten karena keindahannya yang belum terlalu banyak diketahui orang.",
    mainImage: "https://i.imgur.com/DJDoU8x.jpeg",
    images: [
      "https://i.imgur.com/DJDoU8x.jpeg",
      "https://i.imgur.com/fPzrKzx.jpeg",
      "https://i.imgur.com/wOFpYWd.jpeg",
      "https://i.imgur.com/dUDzyhH.jpeg",
      "https://i.imgur.com/HIXGBOV.jpeg"
    ],
    rating: 4.8,
    reviews: 245,
    openingHours: "24 jam",
    bestTimeToVisit: "Mei - September",
    facilities: [
      "Area Parkir",
      "Warung Makan & Kios Minuman",
      "Penginapan & Villa",
      "Toilet & Kamar Bilas",
      "Penyewaan Papan Selancar"
    ],
    activities: [
      "Berselancar (Surfing)",
      "Bermain Pasir & Berenang",
      "Menikmati Sunset",
      "Jalan-jalan Santai",
      "Fotografi Alam"
    ],
    tips: [
      "Datang saat cuaca cerah, terutama pada musim kemarau (Mei-September)",
      "Gunakan kendaraan yang prima karena jalur menuju Sawarna cukup berkelok",
      "Bawa perlengkapan pribadi seperti sunblock, topi, dan air minum",
      "Patuhi aturan lokal dan jaga kebersihan",
      "Pesan penginapan lebih awal, terutama saat musim liburan"
    ],
    nearbyAttractions: [
      { name: "Goa Langir", distance: "5 menit berkendara" },
      { name: "Tanjung Layar", distance: "10 menit berkendara" },
      { name: "Pantai Sawarna", distance: "15 menit berkendara" }
    ]
  },
  {
    id: "karang-bokor",
    name: "Karang Bokor",
    types: ["rock"],
    location: "Karang Bokor, Sawarna",
    description: `
Karang Bokor Sawarna: Surga Tersembunyi di Pesisir Selatan Banten

Desa Sawarna, yang terletak di Kecamatan Bayah, Kabupaten Lebak, Banten, memang menyimpan banyak pesona alam yang menakjubkan. Salah satu destinasi yang mulai mencuri perhatian para pelancong adalah Karang Bokor. Tempat ini menyajikan pemandangan alam yang luar biasa dengan perpaduan tebing karang tinggi, deburan ombak Samudra Hindia, dan panorama laut yang begitu memukau. Bagi pencinta alam dan petualangan, Karang Bokor adalah destinasi yang tak boleh dilewatkan.

Sejarah dan Asal-usul Nama Karang Bokor

Nama "Karang Bokor" berasal dari bentuk batu karang besar yang menyerupai bokor—wadah tradisional berbentuk cekungan yang biasa digunakan dalam upacara adat. Karang besar ini berdiri kokoh di tengah laut, terpisah dari tebing utama oleh ombak yang ganas. Keunikan bentuk dan posisinya membuatnya menjadi ikon khas dari tempat ini.

Dulunya, kawasan ini masih sangat tersembunyi dan belum banyak diketahui wisatawan. Namun seiring berkembangnya pariwisata di Sawarna, Karang Bokor kini mulai dilirik sebagai salah satu spot eksotis yang wajib dikunjungi.

Daya Tarik Karang Bokor

1. Pemandangan Laut Lepas yang Menakjubkan

Dari atas tebing Karang Bokor, pengunjung bisa menyaksikan hamparan Samudra Hindia yang luas dengan ombak besar yang menghantam karang. Suara deburan ombak dan angin laut yang sejuk menciptakan suasana damai yang sulit ditemukan di tempat lain.

2. Tebing Tinggi dan Spot Fotografi Instagramable

Tebing-tebing di Karang Bokor menjulang tinggi dan memberikan latar belakang luar biasa untuk fotografi. Banyak wisatawan datang untuk berburu foto matahari terbit atau terbenam dari titik ini. Jika Anda seorang pecinta fotografi alam, tempat ini akan memanjakan lensa kamera Anda.

3. Jembatan Gantung dan Fasilitas Wisata

Kini, akses menuju Karang Bokor semakin mudah berkat adanya jalur trekking dan jembatan gantung yang menghubungkan area wisata utama dengan titik pandang. Pemerintah daerah dan pengelola setempat juga telah menyediakan beberapa fasilitas seperti gardu pandang, tempat duduk, hingga warung makan sederhana.

Lokasi dan Akses Menuju Karang Bokor

Karang Bokor terletak di Desa Sawarna, sekitar 5 km dari Pantai Sawarna. Untuk mencapainya, pengunjung dapat mengikuti jalur menuju Pantai Ciantir atau Pantai Tanjung Layar, lalu melanjutkan dengan perjalanan kaki atau kendaraan roda dua menuju area Karang Bokor.

Dari Jakarta, perjalanan ke Sawarna memakan waktu sekitar 6-7 jam melalui rute Pelabuhan Ratu. Meski cukup jauh, perjalanan ini akan terbayar lunas dengan keindahan alam yang menanti di ujung jalan.

Tips Berkunjung ke Karang Bokor

• Datang di pagi atau sore hari untuk menghindari terik matahari.
• Gunakan alas kaki yang nyaman untuk trekking menuju lokasi.
• Bawa kamera atau drone untuk mengabadikan keindahan alam dari berbagai sudut.
• Jaga kebersihan dan hormati alam sekitar, karena kawasan ini masih tergolong alami dan minim polusi.

Kesimpulan

Karang Bokor Sawarna adalah salah satu permata tersembunyi di Banten yang menyuguhkan pemandangan alam luar biasa. Tempat ini cocok untuk Anda yang mencari ketenangan, keindahan, dan suasana alami yang masih asri. Dengan akses yang semakin mudah dan fasilitas yang terus dikembangkan, Karang Bokor memiliki potensi besar menjadi destinasi wisata unggulan di pesisir selatan Pulau Jawa.
`,
    mainImage: "https://images.unsplash.com/photo-1501594907352-04cda38ebc29?auto=format&fit=crop&q=80&w=800",
    images: [
      "https://images.unsplash.com/photo-1501594907352-04cda38ebc29?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1465101046530-73398c7f28ca?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1465101178521-c1a9136a3b99?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1464983953574-0892a716854b?auto=format&fit=crop&q=80&w=800"
    ],
    rating: 4.7,
    reviews: 189,
    openingHours: "06:00 - 18:00",
    bestTimeToVisit: "Juni - September",
    facilities: [
      "Area Parkir",
      "Warung Makan",
      "Toilet Umum",
      "Gazebo",
      "Spot Foto"
    ],
    activities: [
      "Fotografi",
      "Matahari Terbenam",
      "Trekking",
      "Bird Watching",
      "Picnic"
    ],
    tips: [
      "Kunjungi saat air surut untuk melihat karang lebih jelas",
      "Bawa kamera untuk fotografi",
      "Gunakan alas kaki anti selip",
      "Perhatikan kondisi cuaca dan ombak"
    ],
    nearbyAttractions: [
      { name: "Pantai Ciantir", distance: "10 menit berkendara" },
      { name: "Goa Langir", distance: "15 menit berkendara" },
      { name: "Tanjung Layar", distance: "20 menit berkendara" }
    ]
  },
  {
    id: "pulo-manuk",
    name: "Pulo Manuk",
    types: ["beach"],
    location: "Pulo Manuk, Sawarna",
    description: "Pantai Pulo Manuk terletak di Desa Sawarna, Kecamatan Bayah, Kabupaten Lebak, Banten. Pantai ini menyuguhkan pemandangan alam yang unik karena berada di antara hutan dan laut, dengan pulau kecil yang berada tak jauh dari bibir pantai—yang menjadi asal-usul namanya: Pulo Manuk (Pulau Burung). Keindahan panorama laut biru, ombak yang lebih tenang dibanding Pantai Ciantir, serta udara sejuk dari hutan sekitar menjadikan Pantai Pulo Manuk sebagai tempat yang ideal untuk bersantai dan menyatu dengan alam. Selain keindahan pantainya, kawasan ini juga memiliki keunikan ekosistem karena berada di dekat kawasan konservasi hutan. Suara burung dan hewan liar dari hutan sekitar sering terdengar, memberikan pengalaman wisata yang berbeda dari pantai pada umumnya.",
    mainImage: "https://i.imgur.com/iC3XmMQ.png",
    images: [
      "https://i.imgur.com/iC3XmMQ.png",
      "https://i.imgur.com/Q7FvHjf.jpeg",
      "https://i.imgur.com/w8jUvjJ.jpeg",
      "https://i.imgur.com/gyNbOMm.jpeg",
      "https://i.imgur.com/UBtzQpu.jpeg",
      "https://i.imgur.com/NoH7f2k.jpeg"
    ],
    rating: 4.9,
    reviews: 156,
    openingHours: "07:00 - 17:00",
    bestTimeToVisit: "Maret - Oktober",
    facilities: [
      "Area Parkir",
      "Warung Makan Lokal",
      "Toilet & Tempat Bilas",
      "Spot Camping",
      "Akses ke Penginapan"
    ],
    activities: [
      "Bersantai & Piknik",
      "Melihat Pulau Kecil",
      "Fotografi Alam",
      "Mengamati Burung & Satwa Liar",
      "Camping"
    ],
    tips: [
      "Datang pagi atau sore hari untuk udara segar dan fotografi",
      "Bawa bekal dan air minum",
      "Gunakan alas kaki yang nyaman",
      "Jaga ketenangan dan kebersihan",
      "Persiapkan perlengkapan camping sendiri"
    ],
    nearbyAttractions: [
      { name: "Pantai Sawarna", distance: "20 menit perjalanan laut" },
      { name: "Karang Bokor", distance: "25 menit perjalanan laut" }
    ]
  },
  {
    id: "curug-sangko",
    name: "Curug Sangko",
    types: ["waterfall"],
    location: "Curug Sangko, Sawarna",
    description: "Curug Sangko adalah air terjun yang tersembunyi di tengah hutan Sawarna. Dengan ketinggian sekitar 25 meter, air terjun ini menawarkan suasana sejuk dan damai yang sempurna untuk melepas penat.",
    mainImage: "https://images.unsplash.com/photo-1502082553048-f009c37129b9?auto=format&fit=crop&q=80&w=800",
    images: [
      "https://images.unsplash.com/photo-1502082553048-f009c37129b9?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1465101046530-73398c7f28ca?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1501785888041-af3ef285b470?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1500534314209-a25ddb2bd429?auto=format&fit=crop&q=80&w=800"
    ],
    rating: 4.5,
    reviews: 132,
    openingHours: "08:00 - 16:00",
    bestTimeToVisit: "Desember - Maret",
    facilities: [
      "Parkir",
      "Warung Makan",
      "Toilet Umum",
      "Gazebo",
      "Tempat Ganti"
    ],
    activities: [
      "Trekking",
      "Fotografi",
      "Berenang",
      "Camping",
      "Picnic"
    ],
    tips: [
      "Bawa air minum dan bekal",
      "Gunakan alas kaki anti selip",
      "Kunjungi saat musim hujan",
      "Bawa baju ganti"
    ],
    nearbyAttractions: [
      { name: "Curug Tujuh", distance: "15 menit berkendara" },
      { name: "Goa Lalay", distance: "20 menit berkendara" }
    ]
  },
  {
    id: "goa-langir",
    name: "Goa Langir",
    types: ["cave"],
    location: "Goa Langir, Sawarna",
    description: "Goa Langir adalah salah satu destinasi wisata alam yang terletak di kawasan Desa Sawarna, Kecamatan Bayah, Kabupaten Lebak, Banten. Goa ini menyimpan keindahan alami yang unik dan menarik, dengan suasana sunyi, dikelilingi oleh pepohonan tropis, dan formasi batu kapur yang memukau. Nama \"Langir\" sendiri berasal dari legenda masyarakat setempat, dan goa ini sering dikaitkan dengan cerita rakyat dan aura mistis yang menambah daya tariknya. Terletak tidak jauh dari Pantai Sawarna, Goa Langir menawarkan perpaduan antara petualangan dan ketenangan. Saat memasuki goa, pengunjung akan disambut dengan hawa sejuk, tetesan air dari stalaktit, serta gemericik air yang mengalir di beberapa bagian goa. Selain keindahan goa, kawasan ini juga memiliki pantai kecil yang tenang dengan pasir putih dan air jernih, menjadikannya destinasi yang sempurna untuk menikmati keindahan alam baik di darat maupun di laut.",
    mainImage: "https://i.imgur.com/wuumijw.jpeg",
    images: [
      "https://i.imgur.com/wuumijw.jpeg",
      "https://i.imgur.com/9JwW5oh.jpeg",
      "https://i.imgur.com/df9elvz.jpeg",
      "https://i.imgur.com/MebbTZE.png",
      "https://i.imgur.com/Wtp19LK.jpeg",
      "https://i.imgur.com/RZWtR2p.jpeg"
    ],
    rating: 4.6,
    reviews: 98,
    openingHours: "08:00 - 16:00",
    bestTimeToVisit: "April - Oktober",
    facilities: [
      "Jalur Akses Pejalan Kaki",
      "Pemandu Wisata Lokal",
      "Tempat Istirahat",
      "Dekat dengan Penginapan",
      "Warung & Fasilitas Umum"
    ],
    activities: [
      "Eksplorasi Goa",
      "Fotografi Alami",
      "Menikmati Suasana Hening",
      "Wisata Edukasi",
      "Meditasi Ringan"
    ],
    tips: [
      "Gunakan alas kaki anti selip",
      "Bawa senter atau headlamp",
      "Datang bersama pemandu lokal",
      "Hindari musim hujan",
      "Jaga kebersihan dan tidak merusak formasi batuan"
    ],
    nearbyAttractions: [
      { name: "Pantai Ciantir", distance: "5 menit berkendara" },
      { name: "Tanjung Layar", distance: "10 menit berkendara" }
    ]
  },
  {
    id: "karang-berem",
    name: "Karang Berem",
    types: ["rock"],
    location: "Karang Berem, Sawarna",
    description: "Karang Berem adalah formasi batuan karang yang unik dengan pemandangan laut lepas. Tempat ini terkenal dengan spot fotografi yang menakjubkan dan pemandangan matahari terbenam yang memukau.",
    mainImage: "https://images.unsplash.com/photo-1501594907352-04cda38ebc29?auto=format&fit=crop&q=80&w=800",
    images: [
      "https://images.unsplash.com/photo-1501594907352-04cda38ebc29?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1465101053361-763ab02bced9?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1465101178521-c1a9136a3b99?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1464983953574-0892a716854b?auto=format&fit=crop&q=80&w=800"
    ],
    rating: 4.6,
    reviews: 145,
    openingHours: "06:00 - 18:00",
    bestTimeToVisit: "Juni - September",
    facilities: [
      "Area Parkir",
      "Warung Makan",
      "Toilet Umum",
      "Gazebo",
      "Spot Foto"
    ],
    activities: [
      "Fotografi",
      "Matahari Terbenam",
      "Trekking",
      "Bird Watching",
      "Picnic"
    ],
    tips: [
      "Kunjungi saat air surut",
      "Bawa kamera untuk fotografi",
      "Gunakan alas kaki anti selip",
      "Perhatikan kondisi cuaca"
    ],
    nearbyAttractions: [
      { name: "Pantai Pasput", distance: "10 menit berkendara" },
      { name: "Karang Taraje", distance: "15 menit berkendara" }
    ]
  },
  {
    id: "karang-taraje",
    name: "Karang Taraje",
    types: ["rock"],
    location: "Karang Taraje, Sawarna",
    description: "Karang Taraje adalah formasi batuan karang yang menyerupai tangga alami. Tempat ini menawarkan pemandangan laut yang menakjubkan dan spot fotografi yang unik.",
    mainImage: "https://images.unsplash.com/photo-1464983953574-0892a716854b?auto=format&fit=crop&q=80&w=800",
    images: [
      "https://images.unsplash.com/photo-1464983953574-0892a716854b?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1465101178521-c1a9136a3b99?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1465101046530-73398c7f28ca?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1501594907352-04cda38ebc29?auto=format&fit=crop&q=80&w=800"
    ],
    rating: 4.5,
    reviews: 128,
    openingHours: "06:00 - 18:00",
    bestTimeToVisit: "Juni - September",
    facilities: [
      "Area Parkir",
      "Warung Makan",
      "Toilet Umum",
      "Gazebo",
      "Spot Foto"
    ],
    activities: [
      "Fotografi",
      "Matahari Terbenam",
      "Trekking",
      "Bird Watching",
      "Picnic"
    ],
    tips: [
      "Kunjungi saat air surut",
      "Bawa kamera untuk fotografi",
      "Gunakan alas kaki anti selip",
      "Perhatikan kondisi cuaca"
    ],
    nearbyAttractions: [
      { name: "Karang Berem", distance: "15 menit berkendara" },
      { name: "Pantai Pasput", distance: "20 menit berkendara" }
    ]
  },
  {
    id: "legon-pari",
    name: "Legon Pari",
    types: ["beach"],
    location: "Legon Pari, Sawarna",
    description: "Legon Pari adalah pantai yang tersembunyi dengan pasir putih dan air jernih. Pantai ini menawarkan ketenangan dan keindahan alam yang masih alami.",
    mainImage: "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&q=80&w=800",
    images: [
      "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1469474968028-56623f02e42e?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1501785888041-af3ef285b470?auto=format&fit=crop&q=80&w=800"
    ],
    rating: 4.7,
    reviews: 167,
    openingHours: "24 jam",
    bestTimeToVisit: "April - Oktober",
    facilities: [
      "Parkir",
      "Warung Makan",
      "Toilet Umum",
      "Tempat Ganti",
      "Gazebo"
    ],
    activities: [
      "Berenang",
      "Fotografi",
      "Matahari Terbenam",
      "Camping",
      "Picnic"
    ],
    tips: [
      "Bawa tabir surya dan air minum",
      "Kunjungi saat pagi atau sore hari",
      "Bawa bekal makanan",
      "Perhatikan kondisi ombak"
    ],
    nearbyAttractions: [
      { name: "Pantai Pasput", distance: "10 menit berkendara" },
      { name: "Karang Berem", distance: "15 menit berkendara" }
    ]
  },
  {
    id: "pantai-pasput",
    name: "Pantai Pasput",
    types: ["beach"],
    location: "Pantai Pasput, Sawarna",
    description: "Pantai Pasput adalah pantai yang terkenal dengan pasir putihnya yang halus dan air jernih. Pantai ini cocok untuk berenang dan menikmati matahari terbenam.",
    mainImage: "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&q=80&w=800",
    images: [
      "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1469474968028-56623f02e42e?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1501785888041-af3ef285b470?auto=format&fit=crop&q=80&w=800"
    ],
    rating: 4.8,
    reviews: 234,
    openingHours: "24 jam",
    bestTimeToVisit: "April - Oktober",
    facilities: [
      "Parkir Luas",
      "Warung Makan",
      "Toilet Umum",
      "Tempat Ganti",
      "Gazebo"
    ],
    activities: [
      "Berenang",
      "Fotografi",
      "Matahari Terbenam",
      "Camping",
      "Picnic"
    ],
    tips: [
      "Bawa tabir surya dan air minum",
      "Kunjungi saat pagi atau sore hari",
      "Bawa bekal makanan",
      "Perhatikan kondisi ombak"
    ],
    nearbyAttractions: [
      { name: "Legon Pari", distance: "10 menit berkendara" },
      { name: "Karang Berem", distance: "15 menit berkendara" }
    ]
  },
  {
    id: "pasir-kolecer",
    name: "Pasir Kolecer",
    types: ["beach"],
    location: "Pasir Kolecer, Sawarna",
    description: "Pasir Kolecer adalah pantai yang terkenal dengan ombaknya yang cocok untuk berselancar. Pantai ini menawarkan pemandangan yang menakjubkan dan suasana yang tenang.",
    mainImage: "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&q=80&w=800",
    images: [
      "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1469474968028-56623f02e42e?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1501785888041-af3ef285b470?auto=format&fit=crop&q=80&w=800"
    ],
    rating: 4.7,
    reviews: 198,
    openingHours: "24 jam",
    bestTimeToVisit: "April - Oktober",
    facilities: [
      "Parkir",
      "Warung Makan",
      "Toilet Umum",
      "Tempat Ganti",
      "Penyewaan Papan Selancar"
    ],
    activities: [
      "Berselancar",
      "Berenang",
      "Fotografi",
      "Matahari Terbenam",
      "Camping"
    ],
    tips: [
      "Bawa tabir surya dan air minum",
      "Sewa papan selancar di lokasi",
      "Kunjungi saat pagi atau sore hari",
      "Perhatikan kondisi ombak"
    ],
    nearbyAttractions: [
      { name: "Pantai Sepang", distance: "10 menit berkendara" },
      { name: "Goa Lalay", distance: "15 menit berkendara" }
    ]
  },
  {
    id: "pantai-sepang",
    name: "Pantai Sepang",
    types: ["beach"],
    location: "Pantai Sepang, Sawarna",
    description: "Pantai Sepang adalah pantai yang terkenal dengan pasir putihnya dan air jernih. Pantai ini cocok untuk berenang dan menikmati matahari terbenam.",
    mainImage: "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&q=80&w=800",
    images: [
      "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1469474968028-56623f02e42e?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1501785888041-af3ef285b470?auto=format&fit=crop&q=80&w=800"
    ],
    rating: 4.6,
    reviews: 176,
    openingHours: "24 jam",
    bestTimeToVisit: "April - Oktober",
    facilities: [
      "Parkir",
      "Warung Makan",
      "Toilet Umum",
      "Tempat Ganti",
      "Gazebo"
    ],
    activities: [
      "Berenang",
      "Fotografi",
      "Matahari Terbenam",
      "Camping",
      "Picnic"
    ],
    tips: [
      "Bawa tabir surya dan air minum",
      "Kunjungi saat pagi atau sore hari",
      "Bawa bekal makanan",
      "Perhatikan kondisi ombak"
    ],
    nearbyAttractions: [
      { name: "Pasir Kolecer", distance: "10 menit berkendara" },
      { name: "Goa Lalay", distance: "15 menit berkendara" }
    ]
  },
  {
    id: "goa-lalay",
    name: "Goa Lalay",
    types: ["cave"],
    location: "Goa Lalay, Sawarna",
    description: "Goa Lalay adalah gua yang terkenal dengan kelelawar yang menghuninya. Gua ini menawarkan pengalaman petualangan yang unik dan pemandangan yang menakjubkan.",
    mainImage: "https://images.unsplash.com/photo-1500534314209-a25ddb2bd429?auto=format&fit=crop&q=80&w=800",
    images: [
      "https://images.unsplash.com/photo-1500534314209-a25ddb2bd429?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1464983953574-0892a716854b?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1465101046530-73398c7f28ca?auto=format&fit=crop&q=80&w=800"
    ],
    rating: 4.5,
    reviews: 145,
    openingHours: "08:00 - 16:00",
    bestTimeToVisit: "April - Oktober",
    facilities: [
      "Parkir",
      "Warung Makan",
      "Toilet Umum",
      "Pemandu Wisata",
      "Peralatan Keselamatan"
    ],
    activities: [
      "Caving",
      "Fotografi",
      "Edukasi",
      "Trekking",
      "Bird Watching"
    ],
    tips: [
      "Gunakan pemandu lokal",
      "Bawa senter",
      "Gunakan alas kaki anti selip",
      "Perhatikan kondisi cuaca"
    ],
    nearbyAttractions: [
      { name: "Pantai Sepang", distance: "15 menit berkendara" },
      { name: "Pasir Kolecer", distance: "15 menit berkendara" }
    ]
  },
  {
    id: "tanjung-layar",
    name: "Tanjung Layar",
    types: ["rock"],
    location: "Tanjung Layar, Sawarna",
    description: "Tanjung Layar adalah formasi batuan karang yang menjorok ke laut, menyerupai layar kapal. Tempat ini menawarkan pemandangan laut lepas yang menakjubkan dan spot fotografi yang unik.",
    mainImage: "https://images.unsplash.com/photo-1464983953574-0892a716854b?auto=format&fit=crop&q=80&w=800",
    images: [
      "https://images.unsplash.com/photo-1464983953574-0892a716854b?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1465101053361-763ab02bced9?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1465101178521-c1a9136a3b99?auto=format&fit=crop&q=80&w=800",
      "https://images.unsplash.com/photo-1501594907352-04cda38ebc29?auto=format&fit=crop&q=80&w=800"
    ],
    rating: 4.8,
    reviews: 212,
    openingHours: "06:00 - 18:00",
    bestTimeToVisit: "Juni - September",
    facilities: [
      "Area Parkir",
      "Warung Makan",
      "Toilet Umum",
      "Gazebo",
      "Spot Foto"
    ],
    activities: [
      "Fotografi",
      "Matahari Terbenam",
      "Trekking",
      "Bird Watching",
      "Picnic"
    ],
    tips: [
      "Kunjungi saat air surut",
      "Bawa kamera untuk fotografi",
      "Gunakan alas kaki anti selip",
      "Perhatikan kondisi cuaca"
    ],
    nearbyAttractions: [
      { name: "Pantai Ciantir", distance: "10 menit berkendara" },
      { name: "Goa Langir", distance: "10 menit berkendara" }
    ]
  }
];

export const getDestinationsByType = (type: 'beach' | 'waterfall' | 'cave' | 'rock') => {
  return destinationsData.filter(dest => dest.types.includes(type));
};

export const getDestinationById = (id: string) => {
  return destinationsData.find(dest => dest.id === id);
};

export const getFeaturedDestinations = () => {
  return destinationsData.slice(0, 8);
};

export const getAllDestinations = () => {
  return destinationsData;
}; 