// Interface RoomType
export interface RoomType {
  id: string;
  name: string;
  description: string;
  price: number;
  capacity: number;
  beds: string;
  bathrooms: number;
  amenities: string[];
  images: string[];
}

// Data room types untuk setiap properti
export const propertyRoomTypes: Record<string, RoomType[]> = {
  "villa-little-hula-hula": [
    {
      id: "laguna",
      name: "Laguna",
      description: "Kamar dengan view kolam renang dan akses langsung ke area outdoor, cocok untuk keluarga.",
      price: 400000,
      capacity: 4,
      beds: "2 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "WiFi", "Kamar Mandi Dalam", "Akses Kolam Renang"],
      images: [
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/ca450ae7_z.jpg",
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/b5ee7db5_z.jpg",
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/4d94180e_z.jpg",
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/0646121a_z.jpg",
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/64fd4bca_z.jpg"
      ]
    },
    {
      id: "rimba",
      name: "Rimba",
      description: "Kamar dengan nuansa alam tropis, cocok untuk keluarga kecil yang ingin suasana asri dan sejuk.",
      price: 350000,
      capacity: 3,
      beds: "1 Queen Bed + 1 Single Bed",
      bathrooms: 1,
      amenities: ["AC", "WiFi", "Kamar Mandi Dalam", "Pemandangan Taman"],
      images: [
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/b13b101f_z.jpg",
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/8b6de0ed_z.jpg",
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/30d27d81_z.jpg",
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/7ca3d3a5_z.jpg",
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/7495a41b_z.jpg"
      ]
    },
    {
      id: "zamrud",
      name: "Zamrud",
      description: "Kamar dengan sentuhan warna hijau zamrud, memberikan ketenangan dan kenyamanan maksimal.",
      price: 350000,
      capacity: 3,
      beds: "1 Queen Bed + 1 Single Bed",
      bathrooms: 1,
      amenities: ["AC", "WiFi", "Kamar Mandi Dalam", "Pemandangan Taman"],
      images: [
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/d7110f07_z.jpg",
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/e36c1b2d_z.jpg",
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/42a43b5c_z.jpg",
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/c0665c79_z.jpg",
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/w647h440x0y0-faed2c0b_z.jpg"
      ]
    },
    {
      id: "paradiso",
      name: "Paradiso",
      description: "Kamar terbesar dengan fasilitas lengkap, cocok untuk rombongan atau keluarga besar.",
      price: 500000,
      capacity: 6,
      beds: "3 Queen Bed",
      bathrooms: 2,
      amenities: ["AC", "WiFi", "2 Kamar Mandi Dalam", "Ruang Keluarga", "Akses Kolam Renang"],
      images: [
        "https://ik.imagekit.io/tvlk/generic-asset/dgXfoyh24ryQLRcGq00cIdKHRmotrWLNlvG-TxlcLxGkiDwaUSggleJNPRgIHCX6/hotel/asset/20036564-17620bb91ad4386e52d8e4ca140bf6b2.jpeg",
        "https://ik.imagekit.io/tvlk/generic-asset/dgXfoyh24ryQLRcGq00cIdKHRmotrWLNlvG-TxlcLxGkiDwaUSggleJNPRgIHCX6/hotel/asset/20036564-c1cd6b0ca8a09d208dfd43e395505f89.jpeg",
        "https://ik.imagekit.io/tvlk/generic-asset/dgXfoyh24ryQLRcGq00cIdKHRmotrWLNlvG-TxlcLxGkiDwaUSggleJNPRgIHCX6/hotel/asset/20036564-25b560e2523f813406448b9c3670da2c.jpeg",
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/30d27d81_z.jpg",
        "https://ik.imagekit.io/tvlk/generic-asset/TzEv3ZUmG4-4Dz22hvmO9NUDzw1DGCIdWl4oPtKumOg=/lodging/54000000/53360000/53350100/53350031/8b6de0ed_z.jpg"
      ]
    }
  ],
  "v4": [
    {
      id: "family",
      name: "Family Room",
      description: "Kamar keluarga yang luas dengan 2 kamar tidur terpisah, masing-masing memiliki kamar mandi dalam. Dilengkapi dengan ruang keluarga yang nyaman dan dapur mini. Cocok untuk keluarga besar atau rombongan.",
      price: 950000,
      capacity: 20,
      beds: "2 King Beds",
      bathrooms: 2,
      amenities: [
        "2 Kamar Tidur",
        "2 Kamar Mandi Dalam",
        "AC",
        "WiFi",
        "TV",
        "Ruang Keluarga",
        "Dapur Mini",
        "Kulkas"
      ],
      images: [
        "https://images.unsplash.com/photo-1613490493576-7fde63acd811?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1616594039964-ae9021a400a0?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1556911220-bff31c812dba?w=1200&auto=format&fit=crop&q=80"
      ]
    },
    {
      id: "single",
      name: "Single Room",
      description: "Kamar single yang nyaman dengan kamar mandi dalam. Dilengkapi dengan AC, WiFi, dan TV. Cocok untuk tamu tunggal atau pasangan.",
      price: 250000,
      capacity: 5,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: [
        "Kamar Mandi Dalam",
        "AC",
        "WiFi",
        "TV",
        "Kipas Angin",
        "Meja Kerja"
      ],
      images: [
        "https://images.unsplash.com/photo-1613490493576-7fde63acd811?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1616594039964-ae9021a400a0?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1556911220-bff31c812dba?w=1200&auto=format&fit=crop&q=80"
      ]
    },
    {
      id: "twin",
      name: "Twin Bed Room",
      description: "Kamar twin bed yang nyaman dengan 2 tempat tidur single. Dilengkapi dengan kamar mandi dalam, AC, WiFi, dan TV. Cocok untuk 2 orang atau keluarga kecil.",
      price: 450000,
      capacity: 8,
      beds: "2 Single Beds",
      bathrooms: 1,
      amenities: [
        "2 Tempat Tidur Single",
        "Kamar Mandi Dalam",
        "AC",
        "WiFi",
        "TV",
        "Kipas Angin",
        "Meja Kerja"
      ],
      images: [
        "https://images.unsplash.com/photo-1613490493576-7fde63acd811?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1616594039964-ae9021a400a0?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1556911220-bff31c812dba?w=1200&auto=format&fit=crop&q=80"
      ]
    }
  ],
  "v5": [
    {
      id: "standard",
      name: "Standard Room",
      description: "Kamar standar Villa Sinar Pelangi menawarkan kenyamanan dengan harga terjangkau. Dilengkapi dengan kipas angin dan kamar mandi bersih yang nyaman, cocok untuk 4-5 orang.",
      capacity: 5,
      beds: "2 Queen Beds",
      bathrooms: 1,
      amenities: ["Kipas Angin", "WiFi", "Kamar Mandi Dalam", "TV"],
      images: [
        "https://i.imgur.com/xjM7Pu6.jpeg",
        "https://i.imgur.com/7EnCW0d.jpeg",
        "https://i.imgur.com/DTBq7Dq.jpeg",
        "https://i.imgur.com/9cttGSs.jpeg",
        "https://i.imgur.com/0aMZ0iY.jpeg"
      ],
      price: 450000
    },
    {
      id: "deluxe",
      name: "Deluxe Room",
      description: "Kamar deluxe Villa Sinar Pelangi menawarkan ruang lebih luas dan fasilitas premium. Dilengkapi dengan AC dan kipas angin untuk kenyamanan maksimal, cocok untuk 4-5 orang.",
      capacity: 5,
      beds: "2 Queen Beds",
      bathrooms: 1,
      amenities: ["AC", "Kipas Angin", "WiFi", "Kamar Mandi Dalam", "TV", "Balkon"],
      images: [
        "https://i.imgur.com/lNcydX5.jpeg",
        "https://i.imgur.com/wZ0cSjQ.jpeg",
        "https://i.imgur.com/WhBRfqs.jpeg",
        "https://i.imgur.com/Nh6XLTg.jpeg",
        "https://i.imgur.com/7EnCW0d.jpeg"
      ],
      price: 550000
    }
  ],
  "v6": [
    {
      id: "standard",
      name: "Standard Room",
      description: "Kamar standar Villa Dekasawarna menawarkan kenyamanan dengan harga terjangkau. Dilengkapi dengan kipas angin dan kamar mandi bersih yang nyaman, cocok untuk 4-5 orang.",
      capacity: 5,
      beds: "2 Queen Beds",
      bathrooms: 1,
      amenities: ["Kipas Angin", "WiFi", "Kamar Mandi Dalam", "TV"],
      images: [
        "https://i.imgur.com/xjM7Pu6.jpeg",
        "https://i.imgur.com/7EnCW0d.jpeg",
        "https://i.imgur.com/DTBq7Dq.jpeg",
        "https://i.imgur.com/9cttGSs.jpeg",
        "https://i.imgur.com/0aMZ0iY.jpeg"
      ],
      price: 200000
    },
    {
      id: "deluxe",
      name: "Deluxe Room",
      description: "Kamar deluxe Villa Dekasawarna menawarkan ruang lebih luas dan fasilitas premium. Dilengkapi dengan AC dan kipas angin untuk kenyamanan maksimal, cocok untuk 4-5 orang.",
      capacity: 5,
      beds: "2 Queen Beds",
      bathrooms: 1,
      amenities: ["AC", "Kipas Angin", "WiFi", "Kamar Mandi Dalam", "TV", "Balkon"],
      images: [
        "https://i.imgur.com/lNcydX5.jpeg",
        "https://i.imgur.com/wZ0cSjQ.jpeg",
        "https://i.imgur.com/WhBRfqs.jpeg",
        "https://i.imgur.com/Nh6XLTg.jpeg",
        "https://i.imgur.com/7EnCW0d.jpeg"
      ],
      price: 350000
    }
  ],
  "v7": [
    {
      id: "standard",
      name: "Standard Room",
      description: "Kamar standar Villa Andrew menawarkan kenyamanan dengan harga terjangkau. Dilengkapi dengan kipas angin dan kamar mandi bersih yang nyaman, cocok untuk 4-5 orang.",
      capacity: 5,
      beds: "2 Queen Beds",
      bathrooms: 1,
      amenities: ["Kipas Angin", "WiFi", "Kamar Mandi Dalam", "TV"],
      images: [
        "https://i.imgur.com/xjM7Pu6.jpeg",
        "https://i.imgur.com/7EnCW0d.jpeg",
        "https://i.imgur.com/DTBq7Dq.jpeg",
        "https://i.imgur.com/9cttGSs.jpeg",
        "https://i.imgur.com/0aMZ0iY.jpeg"
      ],
      price: 250000
    },
    {
      id: "deluxe",
      name: "Deluxe Room",
      description: "Kamar deluxe Villa Andrew menawarkan ruang lebih luas dan fasilitas premium. Dilengkapi dengan AC dan kipas angin untuk kenyamanan maksimal, cocok untuk 4-5 orang.",
      capacity: 5,
      beds: "2 Queen Beds",
      bathrooms: 1,
      amenities: ["AC", "Kipas Angin", "WiFi", "Kamar Mandi Dalam", "TV", "Balkon"],
      images: [
        "https://i.imgur.com/lNcydX5.jpeg",
        "https://i.imgur.com/wZ0cSjQ.jpeg",
        "https://i.imgur.com/WhBRfqs.jpeg",
        "https://i.imgur.com/Nh6XLTg.jpeg",
        "https://i.imgur.com/7EnCW0d.jpeg"
      ],
      price: 350000
    }
  ],
  "v8": [
    {
      id: "standard",
      name: "Standard Room",
      description: "Kamar standar Villa Family menawarkan kenyamanan dengan harga terjangkau. Dilengkapi dengan kipas angin dan kamar mandi bersih yang nyaman, cocok untuk 4-5 orang.",
      capacity: 5,
      beds: "2 Queen Beds",
      bathrooms: 1,
      amenities: ["Kipas Angin", "WiFi", "Kamar Mandi Dalam", "TV"],
      images: [
        "https://i.imgur.com/xjM7Pu6.jpeg",
        "https://i.imgur.com/7EnCW0d.jpeg",
        "https://i.imgur.com/DTBq7Dq.jpeg",
        "https://i.imgur.com/9cttGSs.jpeg",
        "https://i.imgur.com/0aMZ0iY.jpeg"
      ],
      price: 200000
    },
    {
      id: "deluxe",
      name: "Deluxe Room",
      description: "Kamar deluxe Villa Family menawarkan ruang lebih luas dan fasilitas premium. Dilengkapi dengan AC dan kipas angin untuk kenyamanan maksimal, cocok untuk 4-5 orang.",
      capacity: 5,
      beds: "2 Queen Beds",
      bathrooms: 1,
      amenities: ["AC", "Kipas Angin", "WiFi", "Kamar Mandi Dalam", "TV", "Balkon"],
      images: [
        "https://i.imgur.com/lNcydX5.jpeg",
        "https://i.imgur.com/wZ0cSjQ.jpeg",
        "https://i.imgur.com/WhBRfqs.jpeg",
        "https://i.imgur.com/Nh6XLTg.jpeg",
        "https://i.imgur.com/7EnCW0d.jpeg"
      ],
      price: 350000
    },
    {
      id: "jumbo",
      name: "Jumbo Room",
      description: "Kamar jumbo Villa Family adalah pilihan terbaik untuk keluarga besar. Dengan ruang yang sangat luas dan fasilitas lengkap, kamar ini cocok untuk 6-8 orang.",
      capacity: 8,
      beds: "3 Queen Beds",
      bathrooms: 2,
      amenities: ["AC", "Kipas Angin", "WiFi", "2 Kamar Mandi Dalam", "TV", "Balkon", "Ruang Keluarga"],
      images: [
        "https://i.imgur.com/lNcydX5.jpeg",
        "https://i.imgur.com/wZ0cSjQ.jpeg",
        "https://i.imgur.com/WhBRfqs.jpeg",
        "https://i.imgur.com/Nh6XLTg.jpeg",
        "https://i.imgur.com/7EnCW0d.jpeg"
      ],
      price: 450000
    }
  ],
  "villa-mutiara-sawarna": [
    {
      id: "family",
      name: "Family Room",
      description: "Kamar keluarga yang luas dengan 2 kamar tidur terpisah dan kamar mandi dalam. Dilengkapi dengan ruang keluarga yang nyaman dan dapur mini.",
      capacity: 7,
      beds: "2 Queen Beds + 1 Single Bed",
      bathrooms: 1,
      amenities: ["AC", "TV", "Kamar Mandi Dalam", "WiFi", "Balkon", "Mini Kitchen"],
      images: [
        "https://i.imgur.com/oEJpr5X.jpeg",
        "https://i.imgur.com/lqisH0C.jpeg",
        "https://i.imgur.com/oNmci0u.jpeg",
        "https://i.imgur.com/9zWzt7Z.jpeg",
        "https://i.imgur.com/R325714.jpeg"
      ],
      price: 600000
    },
    {
      id: "couple",
      name: "Couple Room",
      description: "Kamar nyaman untuk pasangan dengan pemandangan taman. Dilengkapi dengan tempat tidur queen size dan kamar mandi bersih yang nyaman.",
      capacity: 2,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "TV", "Kamar Mandi Dalam", "WiFi"],
      images: [
        "https://i.imgur.com/n0VVaWi.jpeg",
        "https://i.imgur.com/XepAEE5.jpeg",
        "https://i.imgur.com/jfOhuSi.jpeg",
        "https://i.imgur.com/oHjXCpC.jpeg",
        "https://i.imgur.com/9zWzt7Z.jpeg"
      ],
      price: 350000
    },
    {
      id: "extra",
      name: "Extra Room",
      description: "Kamar tambahan yang nyaman untuk keluarga kecil. Dilengkapi dengan 2 tempat tidur dan kamar mandi dalam.",
      capacity: 4,
      beds: "2 Queen Beds",
      bathrooms: 1,
      amenities: ["AC", "TV", "Kamar Mandi Dalam", "WiFi"],
      images: [
        "https://i.imgur.com/5tWlccP.jpeg",
        "https://i.imgur.com/fti68mz.jpeg",
        "https://i.imgur.com/ylGEmOM.jpeg",
        "https://i.imgur.com/lqisH0C.jpeg",
        "https://i.imgur.com/yDZCZww.jpeg"
      ],
      price: 450000
    }
  ],
  "villa-mega-aura": [
    {
      id: "standard",
      name: "Standar Room",
      description: "Kamar standar nyaman untuk 2 orang, cocok untuk pasangan atau keluarga kecil. Dilengkapi AC, TV, dan kamar mandi dalam.",
      capacity: 2,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "TV", "Kamar Mandi Dalam", "WiFi"],
      images: [
        "https://images.unsplash.com/photo-1618773928121-c32242e63f39?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1590490360182-c33d57733427?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1591088398332-8a7791972843?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1590490359683-658d3d23f972?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1590490359683-658d3d23f972?w=1200&auto=format&fit=crop&q=80"
      ],
      price: 250000
    },
    {
      id: "family",
      name: "Family Room",
      description: "Kamar keluarga luas untuk 4 orang, dengan 2 queen bed, kamar mandi dalam, balkon, dan mini kitchen.",
      capacity: 4,
      beds: "2 Queen Beds",
      bathrooms: 1,
      amenities: ["AC", "TV", "Kamar Mandi Dalam", "WiFi", "Balkon", "Mini Kitchen"],
      images: [
        "https://images.unsplash.com/photo-1618773928121-c32242e63f39?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1590490360182-c33d57733427?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1591088398332-8a7791972843?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1590490359683-658d3d23f972?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1590490359683-658d3d23f972?w=1200&auto=format&fit=crop&q=80"
      ],
      price: 500000
    },
    {
      id: "jumbo",
      name: "Jumbo Room",
      description: "Kamar jumbo sangat luas, cocok untuk rombongan hingga 8 orang. Dilengkapi 3 queen bed, 2 kamar mandi dalam, ruang keluarga, dan balkon.",
      capacity: 8,
      beds: "3 Queen Beds",
      bathrooms: 2,
      amenities: ["AC", "WiFi", "2 Kamar Mandi Dalam", "TV", "Balkon", "Ruang Keluarga"],
      images: [
        "https://images.unsplash.com/photo-1618773928121-c32242e63f39?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1590490360182-c33d57733427?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1591088398332-8a7791972843?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1590490359683-658d3d23f972?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1590490359683-658d3d23f972?w=1200&auto=format&fit=crop&q=80"
      ],
      price: 450000
    }
  ],
  "villa-muara-legon-pari": [
    {
      id: "standard-room",
      name: "Standard Room",
      description: "Kamar nyaman dengan fasilitas lengkap untuk kenyamanan Anda. Dilengkapi dengan kipas angin dan kamar mandi dalam.",
      price: 350000,
      capacity: 2,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["Kipas Angin", "Kamar Mandi Dalam", "WiFi", "View Pantai"],
      images: [
        "https://i.imgur.com/NrQtZBp.jpeg",
        "https://i.imgur.com/NCmW3YT.jpeg",
        "https://i.imgur.com/NrQtZBp.jpeg",
        "https://i.imgur.com/XbUWjkF.jpeg",
        "https://i.imgur.com/ep3rbbc.jpeg"
      ]
    },
    {
      id: "deluxe",
      name: "Deluxe Room",
      description: "Kamar mewah dengan pemandangan pantai yang menakjubkan. Dilengkapi dengan kipas angin dan kamar mandi dalam.",
      price: 450000,
      capacity: 2,
      beds: "1 King Bed",
      bathrooms: 1,
      amenities: ["Kipas Angin", "Kamar Mandi Dalam", "WiFi", "View Pantai"],
      images: [
        "https://i.imgur.com/kk3uXA7.jpeg",
        "https://i.imgur.com/pngviek.jpeg",
        "https://i.imgur.com/rkMGwqt.jpeg",
        "https://i.imgur.com/6bRwedR.jpeg",
        "https://i.imgur.com/mzEJPe8.jpeg"
      ]
    }
  ],
  "villa-widi": [
    {
      id: "standard-room",
      name: "Standard Room",
      description: "Kamar standar yang nyaman dengan pemandangan taman",
      capacity: 2,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi"],
      images: [
        "https://i.imgur.com/B28xTFD.jpeg",
        "https://i.imgur.com/MKYfcag.jpeg",
        "https://i.imgur.com/JYILIYe.jpeg",
        "https://i.imgur.com/gHHUxrK.jpeg",
        "https://i.imgur.com/oRl7XdE.jpeg"
      ],
      price: 250000
    },
    {
      id: "deluxe-room",
      name: "Deluxe Room",
      description: "Kamar deluxe yang luas dengan balkon pribadi",
      capacity: 3,
      beds: "1 King Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kulkas"],
      images: [
        "https://i.imgur.com/Fv9HJPe.jpeg",
        "https://i.imgur.com/l0QsGrb.jpeg",
        "https://i.imgur.com/AJ4MO1L.jpeg",
        "https://i.imgur.com/BtWp6Yr.jpeg",
        "https://i.imgur.com/lf3VX4d.jpeg"
      ],
      price: 450000
    },
    {
      id: "family-room",
      name: "Family Room",
      description: "Kamar keluarga yang sangat luas dengan ruang tamu terpisah",
      capacity: 6,
      beds: "2 King Beds",
      bathrooms: 2,
      amenities: ["AC", "TV LED", "2 Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kitchen", "Ruang Tamu"],
      images: [
        "https://i.imgur.com/eXSGFhd.jpeg",
        "https://i.imgur.com/nwKjG10.jpeg",
        "https://i.imgur.com/SOKaH8H.jpeg",
        "https://i.imgur.com/IYAvuag.jpeg",
        "https://i.imgur.com/YjosayP.jpeg"
      ],
      price: 800000
    }
  ],
  "villa-sawarna-resort-t": [
    {
      id: "standard-room",
      name: "Standard Room",
      description: "Kamar standar yang nyaman dengan pemandangan taman, dilengkapi dengan fasilitas modern untuk kenyamanan Anda",
      capacity: 2,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi"],
      images: [
        "https://i.imgur.com/B28xTFD.jpeg",
        "https://i.imgur.com/MKYfcag.jpeg",
        "https://i.imgur.com/JYILIYe.jpeg",
        "https://i.imgur.com/gHHUxrK.jpeg",
        "https://i.imgur.com/oRl7XdE.jpeg"
      ],
      price: 450000
    },
    {
      id: "deluxe-room",
      name: "Deluxe Room",
      description: "Kamar deluxe yang luas dengan balkon pribadi dan pemandangan laut yang menakjubkan",
      capacity: 3,
      beds: "1 King Bed + 1 Single Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kulkas"],
      images: [
        "https://i.imgur.com/Fv9HJPe.jpeg",
        "https://i.imgur.com/l0QsGrb.jpeg",
        "https://i.imgur.com/AJ4MO1L.jpeg",
        "https://i.imgur.com/BtWp6Yr.jpeg",
        "https://i.imgur.com/lf3VX4d.jpeg"
      ],
      price: 650000
    },
    {
      id: "family-suite",
      name: "Family Suite",
      description: "Suite keluarga yang sangat luas dengan ruang tamu terpisah dan dapur kecil, ideal untuk keluarga besar",
      capacity: 6,
      beds: "2 King Beds + 2 Single Beds",
      bathrooms: 2,
      amenities: ["AC", "TV LED", "2 Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kitchen", "Ruang Tamu"],
      images: [
        "https://i.imgur.com/eXSGFhd.jpeg",
        "https://i.imgur.com/nwKjG10.jpeg",
        "https://i.imgur.com/SOKaH8H.jpeg",
        "https://i.imgur.com/IYAvuag.jpeg",
        "https://i.imgur.com/YjosayP.jpeg"
      ],
      price: 950000
    }
  ],
  "villa-regin-sawarna": [
    {
      id: "deluxe",
      name: "Deluxe Room",
      description: "Kamar deluxe Villa Regin Sawarna menawarkan kenyamanan premium dengan pemandangan area pesawahan yang menenangkan. Dilengkapi dengan fasilitas modern dan ruang yang luas untuk kenyamanan maksimal.",
      capacity: 4,
      beds: "1 King Bed + 1 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "WiFi", "Kamar Mandi Dalam", "TV", "View Pesawahan", "Balkon"],
      images: [
        "https://i.imgur.com/ChtRatA.jpeg",
        "https://i.imgur.com/cjihL8w.jpeg",
        "https://i.imgur.com/55sNE5Y.jpeg",
        "https://i.imgur.com/vZbdMVZ.jpeg",
        "https://i.imgur.com/eOA7T0V.jpeg"
      ],
      price: 450000
    },
    {
      id: "family",
      name: "Family Room",
      description: "Kamar keluarga Villa Regin Sawarna adalah pilihan ideal untuk keluarga besar. Dengan ruang yang sangat luas dan pemandangan pesawahan yang menenangkan, kamar ini menawarkan kenyamanan maksimal untuk seluruh keluarga.",
      capacity: 15,
      beds: "2 King Beds + 1 Queen Bed",
      bathrooms: 2,
      amenities: ["AC", "WiFi", "2 Kamar Mandi Dalam", "TV", "View Pesawahan", "Balkon", "Ruang Keluarga"],
      images: [
        "https://i.imgur.com/Orlvd5N.jpeg",
        "https://i.imgur.com/XG2ijrp.jpeg",
        "https://i.imgur.com/NAy81RD.jpeg",
        "https://i.imgur.com/JWs3N0w.jpeg",
        "https://i.imgur.com/Lv5XWtE.jpeg"
      ],
      price: 750000
    }
  ],
  "villa-sinar-pelangi": [
    {
      id: "standard",
      name: "Standard Room",
      description: "Kamar standar Villa Sinar Pelangi menawarkan kenyamanan dengan harga terjangkau. Dilengkapi dengan kipas angin dan kamar mandi bersih yang nyaman, cocok untuk 4-5 orang.",
      capacity: 5,
      beds: "2 Queen Beds",
      bathrooms: 1,
      amenities: ["Kipas Angin", "WiFi", "Kamar Mandi Dalam", "TV"],
      images: [
        "https://i.imgur.com/xjM7Pu6.jpeg",
        "https://i.imgur.com/7EnCW0d.jpeg",
        "https://i.imgur.com/DTBq7Dq.jpeg",
        "https://i.imgur.com/9cttGSs.jpeg",
        "https://i.imgur.com/0aMZ0iY.jpeg"
      ],
      price: 200000
    },
    {
      id: "deluxe",
      name: "Deluxe Room",
      description: "Kamar deluxe Villa Sinar Pelangi menawarkan ruang lebih luas dan fasilitas premium. Dilengkapi dengan AC dan kipas angin untuk kenyamanan maksimal, cocok untuk 4-5 orang.",
      capacity: 5,
      beds: "2 Queen Beds",
      bathrooms: 1,
      amenities: ["AC", "Kipas Angin", "WiFi", "Kamar Mandi Dalam", "TV", "Balkon"],
      images: [
        "https://i.imgur.com/lNcydX5.jpeg",
        "https://i.imgur.com/wZ0cSjQ.jpeg",
        "https://i.imgur.com/WhBRfqs.jpeg",
        "https://i.imgur.com/Nh6XLTg.jpeg",
        "https://i.imgur.com/7EnCW0d.jpeg"
      ],
      price: 400000
    }
  ],
  "villa-arizky-sawarna": [
    {
      id: "standard",
      name: "Standard Room",
      description: "Kamar standar Villa Arizky menawarkan kenyamanan dengan harga terjangkau. Dilengkapi dengan tempat tidur queen size dan kamar mandi bersih yang nyaman, cocok untuk keluarga kecil.",
      capacity: 4,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["Kipas Angin", "Kamar Mandi Dalam", "WiFi", "View Pantai"],
      images: [
        "https://i.imgur.com/DGHi30T.jpeg",
        "https://i.imgur.com/MbbD0cR.jpeg",
        "https://i.imgur.com/xEM5lSJ.jpeg",
        "https://i.imgur.com/xEM5lSJ.jpeg",
        "https://i.imgur.com/c4tdv00.jpeg"
      ],
      price: 300000
    },
    {
      id: "deluxe",
      name: "Deluxe Room",
      description: "Kamar deluxe Villa Arizky menawarkan ruang lebih luas dan fasilitas premium. Pilihan tepat untuk tamu yang menginginkan kenyamanan ekstra di Legon Pari.",
      capacity: 8,
      beds: "1 King Bed",
      bathrooms: 1,
      amenities: ["AC", "WiFi", "Kamar Mandi Dalam", "TV", "Mini Kulkas", "Balkon", "View Pesawahan"],
      images: [
        "https://i.imgur.com/XQKMbFZ.jpeg",
        "https://i.imgur.com/DabvprB.jpeg",
        "https://i.imgur.com/TASDEI8.jpeg",
        "https://i.imgur.com/zgtnDbS.jpeg",
        "https://i.imgur.com/acr10CW.jpeg"
      ],
      price: 350000
    }
  ],
  "villa-andrew": [
    {
      id: "standard-room",
      name: "Standard Room",
      description: "Kamar standar yang nyaman dengan pemandangan taman",
      price: 350000,
      capacity: 2,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi"],
      images: [
        "https://i.imgur.com/B28xTFD.jpeg",
        "https://i.imgur.com/MKYfcag.jpeg",
        "https://i.imgur.com/JYILIYe.jpeg",
        "https://i.imgur.com/gHHUxrK.jpeg",
        "https://i.imgur.com/oRl7XdE.jpeg"
      ]
    },
    {
      id: "deluxe-room",
      name: "Deluxe Room",
      description: "Kamar deluxe yang luas dengan balkon pribadi",
      price: 450000,
      capacity: 3,
      beds: "1 King Bed + 1 Single Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kulkas"],
      images: [
        "https://i.imgur.com/Fv9HJPe.jpeg",
        "https://i.imgur.com/l0QsGrb.jpeg",
        "https://i.imgur.com/AJ4MO1L.jpeg",
        "https://i.imgur.com/BtWp6Yr.jpeg",
        "https://i.imgur.com/lf3VX4d.jpeg"
      ]
    },
    {
      id: "family-room",
      name: "Family Room",
      description: "Kamar keluarga yang sangat luas dengan ruang tamu terpisah",
      price: 900000,
      capacity: 6,
      beds: "2 King Beds + 2 Single Beds",
      bathrooms: 2,
      amenities: ["AC", "TV LED", "2 Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kitchen", "Ruang Tamu"],
      images: [
        "https://i.imgur.com/eXSGFhd.jpeg",
        "https://i.imgur.com/nwKjG10.jpeg",
        "https://i.imgur.com/SOKaH8H.jpeg",
        "https://i.imgur.com/IYAvuag.jpeg",
        "https://i.imgur.com/YjosayP.jpeg"
      ]
    }
  ],
  "villa-cempaka": [
    {
      id: "standard",
      name: "Standard Room",
      description: "Kamar standar yang nyaman dengan pemandangan sawah. Dilengkapi dengan tempat tidur queen size dan kamar mandi bersih.",
      capacity: 2,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["Kipas Angin", "Kamar Mandi Dalam", "WiFi", "View Pantai"],
      images: [
        "https://i.imgur.com/iAmVH4D.jpeg",
        "https://i.imgur.com/ZWM1vUu.jpeg",
        "https://i.imgur.com/UYjw1CE.jpeg",
        "https://i.imgur.com/R0HR9FM.jpeg",
        "https://i.imgur.com/qKALLdC.jpeg"
      ],
      price: 350000
    },
    {
      id: "deluxe",
      name: "Deluxe Room",
      description: "Kamar deluxe dengan balkon pribadi dan pemandangan sawah yang menenangkan. Dilengkapi dengan tempat tidur king size.",
      capacity: 2,
      beds: "1 King Bed",
      bathrooms: 1,
      amenities: ["AC", "WiFi", "Kamar Mandi Dalam", "TV", "Balkon", "Mini Fridge", "Pemandangan Sawah"],
      images: [
        "https://i.imgur.com/iAmVH4D.jpeg",
        "https://i.imgur.com/ZWM1vUu.jpeg",
        "https://i.imgur.com/UYjw1CE.jpeg",
        "https://i.imgur.com/R0HR9FM.jpeg",
        "https://i.imgur.com/qKALLdC.jpeg"
      ],
      price: 450000
    },
    {
      id: "family",
      name: "Family Room",
      description: "Kamar keluarga yang luas dengan ruang tamu terpisah. Dilengkapi dengan 3 tempat tidur dan 2 kamar mandi.",
      price: 900000,
      capacity: 6,
      beds: "3 Queen Beds",
      bathrooms: 2,
      amenities: ["AC", "WiFi", "2 Kamar Mandi Dalam", "TV", "Ruang Tamu", "Mini Fridge", "Pemandangan Sawah"],
      images: [
        "https://i.imgur.com/iAmVH4D.jpeg",
        "https://i.imgur.com/ZWM1vUu.jpeg",
        "https://i.imgur.com/UYjw1CE.jpeg",
        "https://i.imgur.com/R0HR9FM.jpeg",
        "https://i.imgur.com/qKALLdC.jpeg"
      ]
    }
  ],
  "villa-putri-asih": [
    {
      id: "standard-room",
      name: "Standard Room",
      description: "Kamar standar yang nyaman dengan pemandangan taman",
      capacity: 2,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi"],
      images: [
        "https://i.imgur.com/B28xTFD.jpeg",
        "https://i.imgur.com/MKYfcag.jpeg",
        "https://i.imgur.com/JYILIYe.jpeg",
        "https://i.imgur.com/gHHUxrK.jpeg",
        "https://i.imgur.com/oRl7XdE.jpeg"
      ],
      price: 250000
    },
    {
      id: "deluxe-room",
      name: "Deluxe Room",
      description: "Kamar deluxe yang luas dengan balkon pribadi",
      capacity: 3,
      beds: "1 King Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kulkas"],
      images: [
        "https://i.imgur.com/Fv9HJPe.jpeg",
        "https://i.imgur.com/l0QsGrb.jpeg",
        "https://i.imgur.com/AJ4MO1L.jpeg",
        "https://i.imgur.com/BtWp6Yr.jpeg",
        "https://i.imgur.com/lf3VX4d.jpeg"
      ],
      price: 350000
    },
    {
      id: "family-room",
      name: "Family Room",
      description: "Kamar keluarga yang sangat luas dengan ruang tamu terpisah",
      capacity: 6,
      beds: "2 King Beds",
      bathrooms: 2,
      amenities: ["AC", "TV LED", "2 Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kitchen", "Ruang Tamu"],
      images: [
        "https://i.imgur.com/eXSGFhd.jpeg",
        "https://i.imgur.com/nwKjG10.jpeg",
        "https://i.imgur.com/SOKaH8H.jpeg",
        "https://i.imgur.com/IYAvuag.jpeg",
        "https://i.imgur.com/YjosayP.jpeg"
      ],
      price: 700000
    }
  ],
  "villa-batara": [
    {
      id: "standard-room",
      name: "Standard Room",
      description: "Kamar standar yang nyaman dengan pemandangan taman",
      capacity: 4,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi"],
      images: [
        "https://i.imgur.com/B28xTFD.jpeg",
        "https://i.imgur.com/MKYfcag.jpeg",
        "https://i.imgur.com/JYILIYe.jpeg",
        "https://i.imgur.com/gHHUxrK.jpeg",
        "https://i.imgur.com/oRl7XdE.jpeg"
      ],
      price: 250000
    },
    {
      id: "deluxe-room",
      name: "Deluxe Room",
      description: "Kamar deluxe yang luas dengan balkon pribadi",
      capacity: 4,
      beds: "1 King Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kulkas"],
      images: [
        "https://i.imgur.com/Fv9HJPe.jpeg",
        "https://i.imgur.com/l0QsGrb.jpeg",
        "https://i.imgur.com/AJ4MO1L.jpeg",
        "https://i.imgur.com/BtWp6Yr.jpeg",
        "https://i.imgur.com/lf3VX4d.jpeg"
      ],
      price: 450000
    },
    {
      id: "family-room",
      name: "Family Room",
      description: "Kamar keluarga yang sangat luas dengan ruang tamu terpisah",
      capacity: 6,
      beds: "2 King Beds",
      bathrooms: 2,
      amenities: ["AC", "TV LED", "2 Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kitchen", "Ruang Tamu"],
      images: [
        "https://i.imgur.com/eXSGFhd.jpeg",
        "https://i.imgur.com/nwKjG10.jpeg",
        "https://i.imgur.com/SOKaH8H.jpeg",
        "https://i.imgur.com/IYAvuag.jpeg",
        "https://i.imgur.com/YjosayP.jpeg"
      ],
      price: 800000
    }
  ],
  "villa-matahari": [
    {
      id: "standard-room",
      name: "Standard Room",
      description: "Kamar standar yang nyaman dengan pemandangan taman",
      capacity: 4,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi"],
      price: 350000,
      images: [
        "https://images.unsplash.com/photo-1613490493576-7fde63acd811?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1616594039964-ae9021a400a0?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1556911220-bff31c812dba?w=1200&auto=format&fit=crop&q=80"
      ]
    },
    {
      id: "deluxe-room",
      name: "Deluxe Room",
      description: "Kamar deluxe yang luas dengan balkon pribadi",
      capacity: 4,
      beds: "1 King Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kulkas"],
      price: 450000,
      images: [
        "https://images.unsplash.com/photo-1613490493576-7fde63acd811?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1616594039964-ae9021a400a0?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1556911220-bff31c812dba?w=1200&auto=format&fit=crop&q=80"
      ]
    },
    {
      id: "family-room",
      name: "Family Room",
      description: "Kamar keluarga yang sangat luas dengan ruang tamu terpisah",
      capacity: 6,
      beds: "2 King Beds",
      bathrooms: 2,
      amenities: ["AC", "TV LED", "2 Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kitchen", "Ruang Tamu"],
      price: 800000,
      images: [
        "https://images.unsplash.com/photo-1613490493576-7fde63acd811?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1616594039964-ae9021a400a0?w=1200&auto=format&fit=crop&q=80",
        "https://images.unsplash.com/photo-1556911220-bff31c812dba?w=1200&auto=format&fit=crop&q=80"
      ]
    }
  ],
  "villa-family-sawarna": [
    {
      id: "standard-room",
      name: "Standard Room",
      description: "Kamar standar yang nyaman dengan pemandangan taman",
      capacity: 4,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi"],
      price: 300000,
      images: [
        "https://i.imgur.com/B28xTFD.jpeg",
        "https://i.imgur.com/MKYfcag.jpeg",
        "https://i.imgur.com/JYILIYe.jpeg",
        "https://i.imgur.com/gHHUxrK.jpeg",
        "https://i.imgur.com/oRl7XdE.jpeg"
      ]
    },
    {
      id: "deluxe-room",
      name: "Deluxe Room",
      description: "Kamar deluxe yang luas dengan balkon pribadi",
      capacity: 4,
      beds: "1 King Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kulkas"],
      price: 450000,
      images: [
        "https://i.imgur.com/Fv9HJPe.jpeg",
        "https://i.imgur.com/l0QsGrb.jpeg",
        "https://i.imgur.com/AJ4MO1L.jpeg",
        "https://i.imgur.com/BtWp6Yr.jpeg",
        "https://i.imgur.com/lf3VX4d.jpeg"
      ]
    },
    {
      id: "family-room",
      name: "Family Room",
      description: "Kamar keluarga yang sangat luas dengan ruang tamu terpisah",
      capacity: 6,
      beds: "2 King Beds",
      bathrooms: 2,
      amenities: ["AC", "TV LED", "2 Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kitchen", "Ruang Tamu"],
      price: 700000,
      images: [
        "https://i.imgur.com/eXSGFhd.jpeg",
        "https://i.imgur.com/nwKjG10.jpeg",
        "https://i.imgur.com/SOKaH8H.jpeg",
        "https://i.imgur.com/IYAvuag.jpeg",
        "https://i.imgur.com/YjosayP.jpeg"
      ]
    }
  ],
  "villa-andrew-pasput": [
    {
      id: "standard-room",
      name: "Standard Room",
      description: "Kamar standar yang nyaman dengan pemandangan taman dan pantai. Dilengkapi dengan fasilitas modern untuk kenyamanan Anda.",
      capacity: 4,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi"],
      images: [
        "https://i.imgur.com/B28xTFD.jpeg",
        "https://i.imgur.com/MKYfcag.jpeg",
        "https://i.imgur.com/JYILIYe.jpeg",
        "https://i.imgur.com/gHHUxrK.jpeg",
        "https://i.imgur.com/oRl7XdE.jpeg"
      ],
      price: 350000
    },
    {
      id: "deluxe-room",
      name: "Deluxe Room",
      description: "Kamar deluxe yang luas dengan balkon pribadi dan pemandangan pantai yang menakjubkan. Dilengkapi dengan fasilitas premium untuk kenyamanan maksimal.",
      capacity: 4,
      beds: "1 King Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kulkas"],
      images: [
        "https://i.imgur.com/Fv9HJPe.jpeg",
        "https://i.imgur.com/l0QsGrb.jpeg",
        "https://i.imgur.com/AJ4MO1L.jpeg",
        "https://i.imgur.com/BtWp6Yr.jpeg",
        "https://i.imgur.com/lf3VX4d.jpeg"
      ],
      price: 450000
    }
  ],
  "villa-aki-nini": [
    {
      id: "standard",
      name: "Standard Room",
      description: "Kamar standar yang nyaman dengan pemandangan sawah",
      price: 350000,
      capacity: 2,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "TV", "Kamar Mandi Dalam", "Air Panas"],
      images: [
        "https://i.imgur.com/B28xTFD.jpeg",
        "https://i.imgur.com/MKYfcag.jpeg",
        "https://i.imgur.com/JYILIYe.jpeg",
        "https://i.imgur.com/gHHUxrK.jpeg",
        "https://i.imgur.com/oRl7XdE.jpeg"
      ]
    },
    {
      id: "deluxe",
      name: "Deluxe Room",
      description: "Kamar deluxe dengan pemandangan sawah yang menenangkan",
      price: 450000,
      capacity: 3,
      beds: "1 King Bed",
      bathrooms: 1,
      amenities: ["AC", "TV", "Kamar Mandi Dalam", "Air Panas", "Balkon"],
      images: [
        "https://i.imgur.com/Fv9HJPe.jpeg",
        "https://i.imgur.com/l0QsGrb.jpeg",
        "https://i.imgur.com/AJ4MO1L.jpeg",
        "https://i.imgur.com/BtWp6Yr.jpeg",
        "https://i.imgur.com/lf3VX4d.jpeg"
      ]
    },
    {
      id: "family",
      name: "Family Room",
      description: "Kamar keluarga yang luas dengan pemandangan sawah",
      price: 550000,
      capacity: 4,
      beds: "2 Queen Beds",
      bathrooms: 2,
      amenities: ["AC", "TV", "2 Kamar Mandi Dalam", "Air Panas", "Balkon", "Ruang Tamu"],
      images: [
        "https://i.imgur.com/eXSGFhd.jpeg",
        "https://i.imgur.com/nwKjG10.jpeg",
        "https://i.imgur.com/SOKaH8H.jpeg",
        "https://i.imgur.com/IYAvuag.jpeg",
        "https://i.imgur.com/YjosayP.jpeg"
      ]
    }
  ],
  "villa-pondok-ciantir": [
    {
      id: "standard",
      name: "Standard Room",
      description: "Kamar standar yang nyaman dengan pemandangan taman dan pantai. Dilengkapi dengan fasilitas modern untuk kenyamanan Anda.",
      capacity: 4,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi"],
      images: [
        "https://i.imgur.com/B28xTFD.jpeg",
        "https://i.imgur.com/MKYfcag.jpeg",
        "https://i.imgur.com/JYILIYe.jpeg",
        "https://i.imgur.com/gHHUxrK.jpeg",
        "https://i.imgur.com/oRl7XdE.jpeg"
      ],
      price: 300000
    },
    {
      id: "deluxe",
      name: "Deluxe Room",
      description: "Kamar deluxe yang luas dengan balkon pribadi dan pemandangan pantai yang menakjubkan. Dilengkapi dengan fasilitas premium untuk kenyamanan maksimal.",
      capacity: 4,
      beds: "1 King Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kulkas"],
      images: [
        "https://i.imgur.com/Fv9HJPe.jpeg",
        "https://i.imgur.com/l0QsGrb.jpeg",
        "https://i.imgur.com/AJ4MO1L.jpeg",
        "https://i.imgur.com/BtWp6Yr.jpeg",
        "https://i.imgur.com/lf3VX4d.jpeg"
      ],
      price: 450000
    }
  ],
  "villa-srikandi": [
    {
      id: "standard",
      name: "Standard Room",
      description: "Kamar standar yang nyaman dengan pemandangan pantai. Dilengkapi dengan fasilitas modern untuk kenyamanan Anda.",
      capacity: 4,
      beds: "1 Queen Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi"],
      images: [
        "https://i.imgur.com/B28xTFD.jpeg",
        "https://i.imgur.com/MKYfcag.jpeg",
        "https://i.imgur.com/JYILIYe.jpeg",
        "https://i.imgur.com/gHHUxrK.jpeg",
        "https://i.imgur.com/oRl7XdE.jpeg"
      ],
      price: 300000
    },
    {
      id: "deluxe",
      name: "Deluxe Room",
      description: "Kamar deluxe yang luas dengan balkon pribadi dan pemandangan pantai yang menakjubkan. Dilengkapi dengan fasilitas premium untuk kenyamanan maksimal.",
      capacity: 4,
      beds: "1 King Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kulkas"],
      images: [
        "https://i.imgur.com/Fv9HJPe.jpeg",
        "https://i.imgur.com/l0QsGrb.jpeg",
        "https://i.imgur.com/AJ4MO1L.jpeg",
        "https://i.imgur.com/BtWp6Yr.jpeg",
        "https://i.imgur.com/lf3VX4d.jpeg"
      ],
      price: 450000
    }
  ],
  "villa-sawarna-resort": [
    {
      id: "single-bed",
      name: "Single Bed Room",
      description: "Kamar nyaman dengan single bed, ideal untuk tamu tunggal atau pasangan",
      price: 450000,
      capacity: 2,
      beds: "1 Single Bed",
      bathrooms: 1,
      images: [
        "https://i.imgur.com/B28xTFD.jpeg",
        "https://i.imgur.com/MKYfcag.jpeg",
        "https://i.imgur.com/JYILIYe.jpeg",
        "https://i.imgur.com/gHHUxrK.jpeg",
        "https://i.imgur.com/oRl7XdE.jpeg"
      ],
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi"]
    },
    {
      id: "twin-bed",
      name: "Twin Bed Room",
      description: "Kamar luas dengan twin bed, cocok untuk berdua atau bertiga",
      price: 650000,
      capacity: 3,
      beds: "2 Single Beds",
      bathrooms: 1,
      images: [
        "https://i.imgur.com/Fv9HJPe.jpeg",
        "https://i.imgur.com/l0QsGrb.jpeg",
        "https://i.imgur.com/AJ4MO1L.jpeg",
        "https://i.imgur.com/BtWp6Yr.jpeg",
        "https://i.imgur.com/lf3VX4d.jpeg"
      ],
      amenities: ["AC", "TV LED", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kulkas"]
    },
    {
      id: "superqueen-bed",
      name: "Super Queen Room",
      description: "Kamar mewah dengan super queen bed, dilengkapi ruang tamu terpisah",
      price: 950000,
      capacity: 4,
      beds: "1 Super Queen Bed",
      bathrooms: 2,
      images: [
        "https://i.imgur.com/eXSGFhd.jpeg",
        "https://i.imgur.com/nwKjG10.jpeg",
        "https://i.imgur.com/SOKaH8H.jpeg",
        "https://i.imgur.com/IYAvuag.jpeg",
        "https://i.imgur.com/YjosayP.jpeg"
      ],
      amenities: ["AC", "TV LED", "2 Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kitchen", "Ruang Tamu"]
    }
  ],
  "villa-sinar-matahari-resort": [
    {
      id: "standard",
      name: "Standard Room",
      description: "Kamar standar yang nyaman dengan queen bed premium dan pemandangan pantai yang menakjubkan. Dilengkapi dengan AC, TV LED 43 inch, dan kamar mandi dalam dengan air panas.",
      price: 400000,
      capacity: 2,
      beds: "1 Queen Bed Premium",
      bathrooms: 1,
      images: [
        "https://i.imgur.com/9Hcp40U.jpeg",
        "https://i.imgur.com/JHASLtZ.jpeg",
        "https://i.imgur.com/Khg9u7S.jpeg",
        "https://i.imgur.com/yDZCZww.jpeg",
        "https://i.imgur.com/uh8FQep.jpeg"
      ],
      amenities: ["AC", "TV LED 43 inch", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Mini Kulkas", "Setrika", "Perlengkapan Mandi", "View Pantai"]
    },
    {
      id: "deluxe",
      name: "Deluxe Room",
      description: "Kamar deluxe yang luas dengan king bed premium dan balkon pribadi. Dilengkapi dengan pemandangan pantai yang menakjubkan dan fasilitas modern untuk kenyamanan maksimal.",
      price: 500000,
      capacity: 3,
      beds: "1 King Bed Premium",
      bathrooms: 1,
      images: [
        "https://i.imgur.com/5tWlccP.jpeg",
        "https://i.imgur.com/jfOhuSi.jpeg",
        "https://i.imgur.com/oNmci0u.jpeg"
      ],
      amenities: ["AC", "TV LED 43 inch", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Kulkas", "Setrika", "Perlengkapan Mandi", "View Pantai"]
    }
  ],
  "villa-deka-sawarna": [
    {
      id: "standard-room",
      name: "Standard Room",
      description: "Kamar nyaman dengan twin bed, ideal untuk berdua. Dilengkapi dengan AC, TV LED 43 inch, dan kamar mandi dalam dengan air panas.",
      price: 300000,
      capacity: 2,
      beds: "2 Single Beds",
      bathrooms: 1,
      amenities: ["AC", "TV LED 43 inch", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Mini Kulkas"],
      images: [
        "https://i.imgur.com/B28xTFD.jpeg",
        "https://i.imgur.com/MKYfcag.jpeg",
        "https://i.imgur.com/JYILIYe.jpeg",
        "https://i.imgur.com/gHHUxrK.jpeg",
        "https://i.imgur.com/oRl7XdE.jpeg"
      ]
    },
    {
      id: "deluxe-room",
      name: "Deluxe Room",
      description: "Kamar mewah dengan king size bed dan balkon pribadi. Dilengkapi dengan pemandangan kolam renang dan fasilitas premium.",
      price: 400000,
      capacity: 3,
      beds: "1 King Bed + 1 Single Bed",
      bathrooms: 1,
      amenities: ["AC", "TV LED 55 inch", "Kamar Mandi Dalam", "Air Panas", "WiFi", "Balkon", "Mini Bar", "Ruang Tamu Pribadi"],
      images: [
        "https://i.imgur.com/Fv9HJPe.jpeg",
        "https://i.imgur.com/l0QsGrb.jpeg",
        "https://i.imgur.com/AJ4MO1L.jpeg",
        "https://i.imgur.com/BtWp6Yr.jpeg",
        "https://i.imgur.com/lf3VX4d.jpeg"
      ]
    }
  ]
}; 